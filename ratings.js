...
require('./ibmapm');
//const randomTimeout = require('random');
...
if (process.env.SERVICE_VERSION === 'v2') {
  if (process.env.DB_TYPE === 'mysql') {
    ...
    var hostName = process.env.MYSQL_DB_HOST || 'mysqldb'
    var portNumber = process.env.MYSQL_DB_PORT || '3306'
    var username = process.env.MYSQL_DB_USER || 'root'
...
dispatcher.onGet('/wait', function (req, res) {
  const timeout = 5;
    ...
    if (process.env.DB_TYPE === 'mysql') {
      ...
      connection.connect(function(err) {
          if (err) {
            ...
            console.log(err)
            setTimeout(delayedResponse, 4000 + Math.random() * 1000 );
            return
          }
